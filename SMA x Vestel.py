...
def create_client(ip, port):
    ...
    if not client.connect():
        log_message(f"Verbindung zu {ip}:{port} fehlgeschlagen!")
        return None
    return client

# Relativer Pfad zur Konfigurationsdatei
config_path = os.path.join(os.path.dirname(__file__), 'Konfiguration_SMA x Vestel.ini')
...
def read_registers(client, address, count):
    ...
    if result.isError():
        log_message(f"Fehler beim Lesen der Register {address}: {result}")
        return None
    return result.registers

def PV_Leistung_aktuell():
    if not client_PV:
        log_message("Keine Verbindung zum Wechselrichter!")
        return None
...
def Wallbox(Amps):
    if not client_Wall:
        log_message("Keine Verbindung zur Wallbox!")
        return
